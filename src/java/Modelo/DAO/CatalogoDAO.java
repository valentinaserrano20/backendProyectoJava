...
public class CatalogoDAO {
    ...
    public List<UbicacionDTO> getTiposZona() throws SQLException {
        // Se define la consulta SQL para obtener las zonas residenciales activas
        // (activo = 1) de la tabla tipos_zona.
        String sql = "SELECT id, nombre FROM tipos_zona WHERE activo = 1";

        // try-with-resources: Abre la conexión a la base de datos y ejecuta el
        // statement.
        try (Connection con = Conexion.obtener(); // Se obtiene una conexión activa de MySQL.
            ...
            while (rs.next()) {
                // Mapea la fila creando un nuevo objeto UbicacionDTO con el ID y el nombre de
                // la zona.
    ...
    public List<UbicacionDTO> getSectores() throws SQLException {
        // Sentencia SQL para seleccionar el ID y nombre de los sectores que se
        // encuentran activos (activo = 1)
        String sql = "SELECT id, nombre FROM sectores WHERE activo = 1";

        // try-with-resources: Abre la conexión y prepara la consulta parametrizada de
        // JDBC
        try (Connection con = Conexion.obtener(); // Se conecta a MySQL.
            ...
            // Inicializa la lista dinámica para almacenar los sectores mapeados a
            // UbicacionDTO
            List<UbicacionDTO> lista = new ArrayList<>();
            // Recorre cada registro devuelto por la base de datos
            while (rs.next()) {
                // Instancia el DTO genérico UbicacionDTO con el ID y el nombre del sector y lo
                // agrega a la lista
                lista.add(new UbicacionDTO(rs.getInt("id"), rs.getString("nombre")));
            }
    ...
    // Método que obtiene todos los regímenes o calidades de vivienda activos de la
    // base de datos
    public List<UbicacionDTO> getCalidadesVivienda() throws SQLException {
        // Sentencia SQL para seleccionar el ID y nombre de las calidades de vivienda
        // que se encuentran activas (activo = 1)
    ...
    // Qué hace: Obtiene todos los parentescos activos de la base de datos para
    // mapearlos a objetos UbicacionDTO.
    // Por qué existe: Suministra las opciones de parentesco para la creación de
    // integrantes en el formulario familiar.
    // Qué problema resuelve: Permite poblar dinámicamente las relaciones de
    // parentesco en la UI en lugar de dejarlas fijas en el código.
    public List<UbicacionDTO> getParentescos() throws SQLException {
        ...
        // try-with-resources: Inicializa y administra de forma segura la conexión y el
        // ResultSet.
        try (Connection con = Conexion.obtener(); // Se conecta a MySQL.
                PreparedStatement ps = con.prepareStatement(sql); // Prepara la consulta.
                ResultSet rs = ps.executeQuery()) { // Ejecuta la lectura.
    ...
    // Qué hace: Obtiene todos los grupos sanguíneos activos de la base de datos y
    // los mapea a UbicacionDTO.
    // Por qué existe: Provee las opciones de tipos de sangre para la ficha
    // demográfica y de salud del integrante.
    // Qué problema resuelve: Evita la captura libre de texto o listas harcodeadas
    // propensas a errores ortográficos o inconsistencias médicas.
    public List<UbicacionDTO> getGruposSanguineos() throws SQLException {
        // SELECT recupera el ID y nombre de los grupos sanguíneos de la tabla
        // grupos_sanguineos.
        String sql = "SELECT id, nombre FROM grupos_sanguineos WHERE activo = 1";

        // try-with-resources: Abre de forma segura la conexión JDBC y compila la
        // consulta.
    ...
    // Qué hace: Obtiene todas las nacionalidades activas de la base de datos y las
    // mapea a UbicacionDTO.
    // Por qué existe: Suministra las opciones de país/nacionalidad de origen del
    // familiar en los formularios del plan de emergencia.
    // Qué problema resuelve: Garantiza que los registros de nacionalidad coincidan
    // estrictamente con las opciones homologadas en la base de datos.
    ...
    // y las mapea a UbicacionDTO.
    // Por qué existe: Suministra las opciones de especie animal para la creación y
    // edición de mascotas de la familia.
    // Qué problema resuelve: Permite poblar dinámicamente las especies registradas
    // en MySQL para evitar valores inconsistentes.
    public List<UbicacionDTO> getEspeciesMascota() throws SQLException {
        ...
        // try-with-resources: Administra la conexión física y el PreparedStatement de
        // JDBC.
    ...
    // Por qué existe: Suministra las opciones de género para la creación y edición
    // de mascotas.
    // Qué problema resuelve: Permite poblar dinámicamente los géneros de mascota
    // registrados.
    ...
    // Qué hace: Obtiene todas las amenazas o tipos de riesgo activos de la base de
    // datos.
    // Por qué existe: Provee las opciones de amenazas para el select del formulario
    // de factores de riesgo.
    // Qué problema resuelve: Permite listar dinámicamente las amenazas registradas
    // en la base de datos.
    public List<UbicacionDTO> getAmenazas() throws SQLException {
        ...
        // try-with-resources: Asegura la correcta liberación de la conexión física a
        // MySQL.
        try (Connection con = Conexion.obtener(); // Solicita la conexión.
                PreparedStatement ps = con.prepareStatement(sql); // Prepara la consulta.
                ResultSet rs = ps.executeQuery()) { // Ejecuta el query.
    ...
    // Por qué existe: Suministra las opciones para asociar una vulnerabilidad a un
    // factor de riesgo.
    // Qué problema resuelve: Permite listar dinámicamente las vulnerabilidades de
    // la base de datos.
